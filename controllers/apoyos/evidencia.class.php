...
class evidencia {
	...
	function viewRecurso(){
		...
		$imagen  = $row['IMAGEN'];
		$adjunto = $row['ADJUNTO'];
		$preview = "<img src='".STORAGE_ACCOUNT_CONTAINER."/".CONTAINER_NAME."/media/apoyos/thum_340x220_".$imagen.KEY_ACCESS_BLOBS."'/>";
		$autor = $row['AUTOR'];
		$formato = "";
	   ...
	   $formato = strtoupper($imagen[1]);
		...
		//$dirdownload_file = STORAGE_ACCOUNT_CONTAINER."/".CONTAINER_NAME."/media/apoyos/".$imagen.KEY_ACCESS_BLOBS;
...
WIDTH="340" HEIGHT="220" ShowControls="1" ShowStatusBar="0" ShowDisplay="0" autostart="0"> </EMBED>
	  ...
	  case "TXT":
	   ...
	   /* if(!$this->passport->getPrivilegio($_GET['IDPlan'],'P34')){
		...
		$this->View->replace_content('/\#AUTOR\#/ms' ,htmlentities($autor, ENT_QUOTES, "ISO-8859-1"));
		$this->View->replace_content('/\#DESCRIPCION\#/ms' ,htmlentities($descripcion, ENT_QUOTES, "ISO-8859-1"));	
	 ...
	 $panelcontent ="";
	  $this->Model->getComentarios($_GET['id']);
		   ...
		   //}
			 ...
			 $panelcontent .= '<strong><a href="#" class="comentuser">'.htmlentities($usuario, ENT_QUOTES, "ISO-8859-1").'</a></strong><br/>
						'.$comentario.'
