<?php
include "models/planesoperativo/evidencia.model.php";
...
class evidencia {
	...
	function viewRecurso(){
		...
		$titulo = htmlentities($row['EVIDENCIA']);
		$tiporecurso = $row['TIPO'];
		$imagen  = $row['IMAGEN'];
		$adjunto = $row['ADJUNTO'];
		
		//$preview = "<img src='media/planesoperativos/evidencias/".$imagen."'/>";		
		$ruta_img = "media/planesoperativos/evidencias/thum_340x220_".$imagen;
		$IMAGEN = STORAGE_ACCOUNT_CONTAINER."/".CONTAINER_NAME."/".$ruta_img.KEY_ACCESS_BLOBS;	
		$preview = "<img src='".$IMAGEN."'/>";
	  ...
	  // $linkdescarga = "descargar('".$dirdownload."');return false;";
	    $linkdescarga = STORAGE_ACCOUNT_CONTAINER."/".CONTAINER_NAME."/".$row['ADJUNTO'].KEY_ACCESS_BLOBS;
...
<script type="text/javascript">
    jwplayer("myElement").setup({
		...
		case "mov":
		//$adjunto
        $preview ='<embed width="100%" height="360" name="plugin" src="'.$linkdescarga.'" type="video/quicktime"></embed>
...
<PARAM NAME="FileName" VALUE="'.$linkdescarga.'">
	...
	//   $dirdownload = $row['ADJUNTO'];
	 ...
	 //  $linkdescarga = "descargar('".$dirdownload."');return false;";	
	   ...
	   break;
		...
		$this->View->template = TEMPLATE.'EVIDENCIA.TPL';
		...
		$this->View->replace_content('/\#FECHA\#/ms' ,$row['FECHA_R']->format('Y-m-d'));
	     ...
	     if($this->passport->getPrivilegio($_GET['IDPlan'],$permiso)){			 
		   ...
		   //$btndescargar = '<input  type="button" class="btn btn-warning" onclick="#LINKDESCARGA#" value="Descargar"/>';
		    $btndescargar = '<a type="button"  style="text-decoration: none;" class="btn btn-warning" href="'.$linkdescarga.'" value="Descargar"/>Descargar</a>';
		 }else{
		 	$btndescargar = " ";
		 }
