...
$worksheet->write($celda, 1, 'Línea estratégica '.$contlinea.': '.$row['LINEA'],$format_our_green);
 $worksheet->mergeCells($celda, 1, $celda, 7); 
			   ...
			   foreach($Model->objetivos as $resultados){
					...
					$worksheet->write($celda,4,'Objetivo estratégico',$format_our_green2);
                   ...
                   //
					...
					//$worksheet->writeString($celda,2,$resultados['OBJETIVO'],$multipleLineDataFormat);
					$worksheet->mergeCells($celda, 2, $celda, 3);	
...
$worksheet->write(4, 1, 'Análisis Interno:',$format_our_green4);//comentar
...
$worksheet->mergeCells(5, 1, 5, 7);
  ...
  $contcelda=7;
	           ...
	           $worksheet->write($contcelda, 1, 'Debilidades',$format_our_green);
                ...
                $contcelda++;//comentar                  
                $worksheet->write($contcelda, 1, 'Análisis Externo:',$format_our_green4);//comentar
...
foreach($Model->lineas as $row){
 ...
 $worksheet->write($celda, 1, 'Línea estratégica '.$contlinea.':',$format_our_green);
...
$celda++;
 ...
 $worksheet->write($celda, 1, "Objetivos Estratégicos",$format_our_green2);
 $worksheet->mergeCells($celda, 1, $celda, 7);
