...
$worksheet->write(3, 1, 'Resultado: Acción propuesta que ayuda al cumplimiento de algún Objetivo Estratégico del Plan Estratégico de la universidad y el responsable de lograr dicho resultado');


$worksheet->write(4, 1, 'Medios: Tareas a llevar a cabo para el logro del objetivo táctico y los responsables de llevarlas a cabo.');

$worksheet->write(5, 1, 'Evidencias propuestas: Elementos físicos que ayudan a evaluar el cumplimiento de los objetivos tácticos. Ejemplos: reportes, fotografías, informes de reuniones, correos, etc.');
...
$worksheet->write($celda, 1, 'Línea estratégica '.$contlinea.': '.$row['LINEA']);
$worksheet->setMerge($celda, 1, 1, 10);
			   ...
			   foreach($this->Model->objetivos as $resultados){
					...
					$worksheet->write($celda,2,'Resultado');
			        $worksheet->mergeCells($celda, 2, $celda, 3);
			        //  $worksheet->setRow($celda,26);	
			        
					$worksheet->write($celda,4,'Objetivo estrategico');
				    
					$worksheet->write($celda,5,'Responsable');
					 $worksheet->mergeCells($celda, 5, $celda, 6);
					
					
					$worksheet->write($celda,7,'Evidencias propuestas');
                ...
                //    $worksheet->write($columnas[$fila].$celda,$periodo['PERIODO']);	
                    $worksheet->write($celda,$columnas[$fila],$periodo['PERIODO']);
               ...
               //  $sheet->setCellValue($columnas[$fila].$celda, $porcentaje);
				  ...
				  //  $worksheet->mergeCells($celda, 5, $celda, 6);
					...
					$worksheet->write($celda,3,"Medios");
                ...
                // $sheet->setCellValue($columnas[$fila].$celda, $porcentaje);
	             ...
	             $contobjetivo++;          
				  ...
				  $worksheet->write($celda,2,"COMENTARIOS",$multipleLineDataFormat);				
                ...
                $worksheet->write($celda,$columnas[$fila],"COMENTARIOS",$multipleLineDataFormat);
 ...
 $worksheet->write($celdaperiodo,$columnas[$fila],$usuario.'         '.$comentario['FECHA_R'],$multipleLineDataFormat);
   ...
   $worksheet->write($celdaperiodo,$columnas[$fila],$comentario['COMENTARIO'],$multipleLineDataFormat);                
				...
				//	$sheet->mergeCells('D'.$filacoment.':H'.$filacoment);
				   ...
				   $worksheet->write($filacoment,3,$usuario.'         '.$comentario['FECHA_R'],$multipleLineDataFormat);
...
$worksheet->write(5, 1, 'Áreas de oportunidad');
$worksheet->mergeCells(5, 1, 5, 7);
  ...
  $contcelda=7;
	           ...
	           $worksheet->write($contcelda, 1, 'Fortalezas');
                $worksheet->mergeCells($contcelda, 1, $contcelda, 7);
                   ...
                   $worksheet->write($contcelda, 2, 'COMENTARIOS');
                  $worksheet->mergeCells($contcelda, 2, $contcelda, 7);
...
foreach($this->Model->lineas as $row){
 ...
 $worksheet->write($celda, 1, 'Línea estratégica '.$contlinea.':');
...
$celda++;
 ...
 $worksheet->write($celda, 1, "Objetivos Estratégicos");
 $worksheet->mergeCells($celda, 1, $celda, 7);
...
$worksheet->write(1, 1, $jerarquia,$multipleLineDataFormatplab);
 $worksheet->mergeCells(1, 1, 1, 7);
   ...
   $worksheet->write(4, 1, "Comentarios Generales",$multipleLineDataFormatplab);
  ...
  $contcelda=7;
                         ...
                         $worksheet->mergeCells($contcelda, 2, $contcelda, 7);
							...
							$usuario = $this->Model->getResponsable($comentario['PK_USUARIO']);				    
							 ...
							 $worksheet->write($contcelda, 2, $usuario.'         '.$comentario['FECHA_R']);
							 $worksheet->mergeCells($contcelda, 2, $contcelda, 7);
