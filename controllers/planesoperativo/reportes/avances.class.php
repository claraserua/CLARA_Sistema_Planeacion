<?php
include "models/planesoperativo/reportes/avances.model.php";
...
class avances {
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/reportes/po/AVANCES.TPL');
	...
	function Reporte(){
		...
		$html='<div class="datagrid" style="overflow-x:auto;"><table style="width: 150%;">';
		$numplanes = sizeof($this->Model->planes); 
		 ...
		 if($numplanes != 0){
		 	...
		 	$html.='<tr><td colspan="5"><table style="width: 100%;">';	
		 	
		 	
		 	
		 		$html.='<td  style="border:1px #999999 solid; background:#CCC; color:#333;"   width="5%"><strong>Plan</td></strong>';
			...
			$html.='<td style="border:1px #999999 solid; background:#CCC; color:#333;" width="8%"><strong>Indicador 2024</strong></td>';
			...
			$html.='<td style="border:1px #999999 solid; background:#CCC; color:#333;" width="5%"><strong>Meta Anual</strong></td>';
		 	...
		 	$numperiodos = sizeof($this->Model->periodos);
		 	   if($numperiodos != 0){
				 ...
				 $html.='<td  style="border:1px #999999 solid; background:#CCC; color:#333;" width="5%" ><strong>Comentarios '.htmlentities($periodo['PERIODO'], ENT_QUOTES, "ISO-8859-1").'</strong></td>';	
		 	...
		 	//$html.='<tr>';
		 	      ...
		 	      if($numlineas != 0){
				 	   ...
				 	   /*  $porcentaje1 = $this->Model->getPeriodoAvanceRes($plan['PK1'],1,$resultado['PK1']);	   
				 	     $porcentaje2 = $this->Model->getPeriodoAvanceRes($plan['PK1'],2,$resultado['PK1']);  
				...
				$metaByObjT = $ArrayIndicadoresMetasByObjT[1]; 
		 	    ...
		 	    $html.='<td  width="8%" style="border:1px #999999 solid; font-size:12px; '.$color.'  '.$backgraund.'  " colspan="">'.$indicadorByObjE.'</td>';
				 $html.='<td  width="8%" style="border:1px #999999 solid; font-size:12px; '.$color.'  '.$backgraund.'  " colspan="">'.$metaByObjE.'</td>';   
				...
				$html.='<td  width="5%" style="border:1px #999999 solid; font-size:12px; '.$color.'  '.$backgraund.' " colspan="">'.$this->Model->getResponsable($resultado['PK_RESPONSABLE']).'</td>';	 	 	
					...
					if($numcomentarios != 0){
		 	          $countcom=1; 
		 	           	$html.='<td  width="5%" colspan="" style="border:1px #999999 solid; font-size:12px; '.$color.'  '.$backgraund.'">';
		 	          	foreach($this->Model->comentariosp as $comentario){	 	           	
		 	       
		 	       
		 	       $usuario = $this->Model->getResponsable($comentario['PK_USUARIO']);  
		 	       $responfech = '<span><font color="black"><strong>'.$usuario.'         '.$comentario['FECHA_R']->format('Y-m-d').'</strong></font></span>';
		 	       		 	         	
		 	           	
	   $html.="\n ".$responfech.'         '.$countcom++.'.'.htmlentities($comentario['COMENTARIO'], ENT_QUOTES, "ISO-8859-1");
		 	       ...
		 	       $html.='</td>';      
					 	...
					 	$html.='<td  width="5%" style="border:1px #999999 solid; font-size:12px;" colspan=""></td>';
					 	
					 }  
		 	           
		 	           
		 	           $z++;
		 	        	//$html.='<td  width="5%" colspan=""></td>';
