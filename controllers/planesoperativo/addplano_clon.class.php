<?php
include "models/planesoperativo/addplano_clon.model.php";
...
class addplano_clon {
	...
	function addplano_clon() {
		
     $this->passport = new Authentication();
	 $this->menu = new Menu();
	 $this->Model = new addplano_clonModel();
	...
	function error(){
	 ...
	 function loadContent(){
		...
		$contenido = $this->View->Template(TEMPLATE.'modules/planesoperativo/ADDPLAN_CLON.TPL');
		
		$idplan =  strtoupper(substr(uniqid('PO'), 0, 15));
		
		
		$contenido =  $this->View->replace('/\#IDPLAN_CLONAR\#/ms' ,$_GET['IDPlan'],$contenido);
		...
		$this->View->replace_content('/\#CONTENT\#/ms' ,$contenido);
         ...
         function GuardarPlan(){
		     ...
		     $this->Model->idplan_clonar = $_POST['idplan_clonar'];
