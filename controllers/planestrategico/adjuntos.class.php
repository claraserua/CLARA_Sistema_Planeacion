...
class adjuntos {
  ...
  var $View;
	 ...
	 function Buscar(){
		...
		foreach($this->Model->archivos as $row){
		    ...
		    if(trim($row['TIPO'])=="IMG"){
			  $imagen = STORAGE_ACCOUNT_CONTAINER."/".CONTAINER_NAME."/media/planesestrategicos/thum_170x170_".$row['IMAGEN'].KEY_ACCESS_BLOBS;
			}else{
				$imagen = $row['IMAGEN'];
			}
	...
	/*if ($imageupload->uploaded) {
         ...
         $imageupload->Process($this->targetPathumbail);
            if ($imageupload->processed) {
							$object_Blob->cargandoBlob($this->image, getcwd()."/".$this->targetPathumbail.$this->image, substr(("/".$this->targetPathumbail), 0, -1));//comienza con  /  la ruta y termina sin /
					//unlink( $this->adjunto);	
                 return TRUE;
	...
	function Eliminar(){
		try {
			$ruta_eliminar = CONTAINER_NAME."/media/planesestrategicos";
