  ...
  var $View;
	...
	function Buscar(){
		...
		$imagen = ($row['IMAGEN'] == "avatar.jpg") ? "user.png" : "thum_40x40_".$row['IMAGEN'];
		   ...
		   if($row['ACTIVO']==1){ $activo = '&nbsp;&nbsp;<img src="media/usuarios/on.png" style="width:10px; height:10px" title="Usuario Activo" />'; }else{ $activo = " "; }
		   ...
		   if($row['DISPONIBLE']!=0 || $this->passport->privilegios->hasPrivilege('P39')){  
			 ...
			 $recurso .= '<tr '.$clase.'>
								<td><img src="media/usuarios/'.$imagen.'" style="width:40px; height:40px"  title="Imagen" onerror="this.src=\'media/usuarios/user.png\';">'.$activo.'</td>
								<td><h3>'.$row['PK1'].'</h3></td>
			    ...
			    if($this->passport->privilegios->hasPrivilege('P12')){
								...
								$recurso .='<button class="btn btn-small" onclick="EliminarUsuario(\''.$row['IMAGEN']."','".$row['PK1'].'\',false)" ><i class="icon-trash"></i> Borrar</button>&nbsp;';
								     }
								}
	...
	function Eliminar(){
		$ruta_eliminar = "media/usuarios";
    $file_eliminar = $_POST['file'];
		$id = $_POST['id'];
		unlink($ruta_eliminar."/".'thum_40x40_'.$file_eliminar);
		unlink($ruta_eliminar."/".'thum_100x100_'.$file_eliminar);
		
		/*echo "<script>alert('ruta_eliminar: '.$ruta_eliminar.');</script>";
    echo "<script>alert('file_eliminar: '.$file_eliminar.');</script>";
    echo "<script>alert('idevidencia: '.$id.');</script>";
		$this->object_Blob->deleteBlob($ruta_eliminar, $file_eliminar);
    $this->object_Blob->deleteBlob($ruta_eliminar,"thum_40x40_".$file_eliminar);
    $this->object_Blob->deleteBlob($ruta_eliminar,"thum_100x100_".$file_eliminar);
		*/
