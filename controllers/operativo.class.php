<?php
include "models/operativo.model.php";
...
class operativo {
	...
	function operativo() {
	 $this->menu = new Menu("F2","SF21");
	 $this->nodos = new Niveles("filtro");
	 $this->Model = new operativoModel();
	...
	function error(){
	 ...
	 $section = TEMPLATE.'modules/planesoperativo/GRDPLAN.TPL';
	 $section = $this->View->loadSection($section);
	 ...
	 function Buscar(){
	   ...
	   $urlEditObjetivos="?execute=planesoperativo/editobjetivos&method=default&Menu=F2&SubMenu=SF21&IDPlan=".$row['PK1']."&IDPlanE=".$row['PK_PESTRATEGICO']."&IDJerarquia=".$row['PK_JERARQUIA'];
			...
			$recurso .= '<tr '.$clase.'>
								...
								<a href="'.$url.'" class="operativoLoading"><span  style="float:left; margin-top:8px;"><h3>'.htmlentities($row['TITULO'], ENT_QUOTES, "ISO-8859-1").'</h3></span></a>
								...
								<div class="btn-group" style="float:left; margin-left:10px;">
								  ...
								  <ul class="dropdown-menu">';
								   ...
								   $recurso .='<li><a href="'.$url.'" target="_blank"><i class=" icon-arrow-right"></i> Abrir Externa</a></li><li class="divider"></li>';
								   
								   
								   if($this->passport->privilegios->hasPrivilege('P211')){//clon
									$recurso .='<li><a href="?execute=planesoperativo/addplano_clon&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanEstrategico='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'" ><i class="icon-file"></i>Clonar</a></li><li class="divider"></li>';
									//'?execute=planesoperativo/addplano&method=default&Menu=F2&SubMenu=SF21&IDPlanEstrategico='+idplane+'&IDJerarquia='+idjerarquia;
								}
								...
								$recurso .='  <li><a href="#" onClick="javascript:ExportarExcel(\''.$row['PK1'].'\',\''.$row['PK_PESTRATEGICO'].'\',\''.htmlentities($row['TITULO'], ENT_QUOTES, "ISO-8859-1").'\');return false;" ><i class="icon-file"></i> Exportar Excel</a></li>';
								//$recurso .='  <li><a href="?execute=planesoperativo/exportexcel&IDPlan='.$row["PK1"].'&IDPlanE='.$row["PK_PESTRATEGICO"].'""  ><i class="icon-file"></i> Exportar Excel</a></li>';
								
								  }
								  
								  
               						$recurso .='
<li><a href="?execute=planesoperativo/evidenciasplan&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'" ><i class="icon-folder-open"></i> Evidencias</a></li> <li class="divider"></li>';

								  
								  
								 if($this->passport->getPrivilegio($row['PK1'],'P35')){
								
								$body = htmlentities($row['TITULO']).': http://13.84.158.61/app/planeacion/shared.app?execute=PO51927619CDB8A/PE518EA2F51B620/PO';		
...
<li><a href="?execute=planesoperativo/asignaciones&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'#&p=1&s=25&sort=1&q=" ><i class="icon-th-list"></i> Asignaciones</a></li>';
								...
								if($this->passport->privilegios->hasPrivilege('P40')){
									$recurso .='<li><a href="?execute=planesoperativo/editestado&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'" ><i class="icon-flag"></i> Cambiar estado</a></li><li class="divider"></li>';
								}								
								
								
								
								if($this->passport->privilegios->hasPrivilege('P212'))
								{
									$recurso .='<li><a href="?execute=planesoperativo/CedulasRetroalimentacion&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&IDPlanE='.$row['PK_PESTRATEGICO'].'&IDJerarquia='.$row['PK_JERARQUIA'].'" ><i class="icon-file"></i> Cedula de Retroalimentaci&oacute;n</a></li><li class="divider"></li>';
								}
			


                 if($this->passport->privilegios->hasPrivilege('P006')){
								$recurso .='<li><a href="#" onClick="javascript:ArchivarPlanOperativo(\''.$row['PK1'].'\',false);return false;"><i class="icon-inbox"></i> Archivar</a></li>';
								}
...
<li><a href="?execute=planesoperativo/editplano&method=default&Menu=F2&SubMenu=SF21&IDPlan='.$row['PK1'].'&nodo='.$row['PK_JERARQUIA'].'" ><i class="icon-pencil"></i> Editar</a></li>';
								...
								$recurso .='<li><a href="'.$urlEditObjetivos.'" ><i class=" icon-pencil"></i> Editar Resultados</a></li>';
								}
