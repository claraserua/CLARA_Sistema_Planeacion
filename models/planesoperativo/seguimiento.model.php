...
$buscar = " AND REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER(EVIDENCIA),'�','a'), '�','e'),'�','i'),'�','o'),'�','u'),'�','n') LIKE  REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER('%".$_GET['q']."%'),'�','a'), '�','e'),'�','i'),'�','o'),'�','u'),'�','n') ";
...
";	
	...
	function getNumeroEvidencias($idobjetivo){
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_EVIDENCIAS WHERE PK_OTACTICO = '$idobjetivo' AND ADJUNTO <> 'NULL'";
	...
	function getImagen($id){
		$sql = "SELECT IMAGEN, NOMBRE, APELLIDOS FROM PL_USUARIOS WHERE PK1 = '$id'";   
	...
	function getPlanOperativo($id){
	   ...
	   function getLineaPlane($id){
		$sql = "SELECT ORDEN, LINEA,PK1 FROM PL_PESTRATEGICOS_LINEASE WHERE PK1='$id'";   
		$row = database::getRow($sql);
		if($row){
			return $row;
		}else{
       ...
       function getLineasPlane($id){
		...
		$this->lineas = array();
        $sql = "SELECT PK1, LINEA FROM PL_PESTRATEGICOS_LINEASE WHERE PK_PESTRATEGICO = '$id' ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   $this->lineas[] = $row;
        ...
        $sql = "SELECT ORDEN, OBJETIVO FROM PL_PESTRATEGICOS_OBJETIVOSE WHERE PK1 = '$id' ";	
		$row = database::getRow($sql);
		if($row){
			return $row;
		}else{
		...
		if($row){
         ...
         $sql = "SELECT PK1, OBJETIVO FROM PL_POPERATIVOS_OBJETIVOST WHERE PK_POPERATIVO = '$idplan' AND  PK_LESTRATEGICA = '$idlinea' ORDER BY ORDEN ";	
	     $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		 ...
		 $this->objetivos = array();
         $sql = "SELECT ORDEN, OBJETIVO, PK_OESTRATEGICO FROM PL_POPERATIVOS_OBJETIVOST WHERE PK_POPERATIVO = '$idplan' AND  PK_LESTRATEGICA = '$idlinea' AND PK1  = '$idresultado' ORDER BY ORDEN ";	
	     $row = database::getRow($sql);
		...
		$sql = "SELECT APELLIDOS, NOMBRE FROM PL_USUARIOS WHERE PK1 = '$id' ";	
		$row = database::getRow($sql);
		if($row){
		...
		$this->objetivos = array();
        $sql = "SELECT PK1, PK_OESTRATEGICO, OBJETIVO, PK_RESPONSABLE, PK_LESTRATEGICA  FROM PL_POPERATIVOS_OBJETIVOST WHERE PK_POPERATIVO = '$idPLAN' AND PK_LESTRATEGICA = '$idlINEA' ORDER BY ORDEN";	
	   ...
	   foreach($rows as $row){
	    ...
	    $this->objetivos[] = $row;
		 ...
		 function getIndicadoresMetas($id){//**
			...
			$sql = "SELECT INDICADOR, META FROM PL_POPERATIVOS_INDICADORESMETAS WHERE PK_OTACTICO = '$id' ORDER BY ORDEN";	
			$rows = database::getRows($sql);
		...
		function getIndicadoresByObj($idObjT,$idObjE){
			...
			$sql = "SELECT INDICADOR,META 
					...
					AND A.PK_OESTRATEGICO = '$idObjE'
					AND B.PK_OTACTICO = '$idObjT'";	
			$rows = database::getRows($sql);
		...
		$this->medios = array();
        $sql = "SELECT PK1, MEDIO, PK_RESPONSABLE FROM PL_POPERATIVOS_MEDIOS WHERE PK_OTACTICO = '$id' ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		...
		function getEvidencias($id){//no se usa?
	...
	//	$buscar = " REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE( LOWER(CONVERT(VARCHAR(MAX), EVIDENCIA)),'�','a'), '�','e'),'�','i'),'�','o'),'�','u'),'�','n') LIKE REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(REPLACE(LOWER('%promoci�n%'),'�','a'), '�','e'),'�','i'),'�','o'),'�','u'),'�','n') ";
			
		//'%promocion%'
		
		$this->evidencias = array();
		
       $sql = "SELECT PK1, PK_LESTRATEGICA, IMAGEN, FECHA_R, TIPO, EVIDENCIA, PK_OTACTICO, AUTOR, DESCRIPCION FROM PL_POPERATIVOS_EVIDENCIAS WHERE EVIDENCIA LIKE  '%".$linea."%'  AND PK_POPERATIVO = '$plan' ORDER BY ORDEN ";	
       ...
       if(isset($_GET['q']) && $_GET['q']!= ""){ 
		...
		$this->evidencias = array();
        $sql = "SELECT PK1, IMAGEN, FECHA_R, TIPO, EVIDENCIA, PK_LESTRATEGICA, ADJUNTO, PK_OTACTICO, DESCRIPCION, AUTOR FROM PL_POPERATIVOS_EVIDENCIAS WHERE PK_OTACTICO = '$id' AND PK_POPERATIVO = '$plan' $buscar ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		...
		$sql = "SELECT AREA FROM PL_POPERATIVOS_AREAS WHERE PK_POPERATIVO = '$planoperativo' ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
	    ...
	    $planoperativo =  $_GET['IDPlan'];
		$sql = "SELECT FORTALEZA FROM PL_POPERATIVOS_FORTALEZAS WHERE PK_POPERATIVO = '$planoperativo' ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
	    ...
	    $planoperativo =  $_GET['IDPlan'];
		$sql = "SELECT AMENAZAS FROM PL_POPERATIVOS_AMENAZAS WHERE PK_POPERATIVO = '$planoperativo' ORDER BY ORDEN";
	   ...
	   foreach($rows as $row){
	    ...
	    $planoperativo =  $_GET['IDPlan'];
		$sql = "SELECT OPORTUNIDADES FROM PL_POPERATIVOS_OPORTUNIDADES WHERE PK_POPERATIVO = '$planoperativo' ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$id' AND ENVIADO IN ('1','2') ORDER BY ORDEN";	
		...
		$sql = "SELECT PK1, FECHA_I, ORDEN, ENVIADO, PERIODO FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$id'  ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		...
		$this->periodos = array();       
		$sql = "SELECT PK1, ENVIADO, ORDEN, PERIODO FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$id' AND FECHA_I <= '$fecha_i' ORDER BY ORDEN";	
	    $rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_OBJETIVOST_COMENTARIOS WHERE PK_OTACTICO = '$id'";	
		$num = database::getNumRows($sql);  	
		return $num;
		}
        ...
        $sql = "SELECT PK1 FROM PL_POPERATIVOS_OBJETIVOST WHERE PK_LESTRATEGICA = '$idlinea' AND PK_POPERATIVO = '$idPO'";	
		$num = database::getNumRows($sql);  	
		return $num;
        ...
        $sql = "SELECT PK1 FROM PL_POPERATIVOS_EVIDENCIAS WHERE PK_LESTRATEGICA = '$idlinea' AND PK_POPERATIVO = '$idPO'  AND  ADJUNTO <> 'NULL'  AND  ADJUNTO <> ''";	
		$num = database::getNumRows($sql);  	
		return $num;
        ...
        $sql = "SELECT  PK1  FROM PL_POPERATIVOS_EVIDENCIAS WHERE PK_LESTRATEGICA = '$idlinea' AND PK_POPERATIVO = '$idPO' AND PK_OTACTICO = '$resultado' AND  ADJUNTO <> 'NULL'  AND  ADJUNTO <> '' ";	
		$num = database::getNumRows($sql);  	
		return $num;
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_OBJETIVOST_COMENTARIOS_PERIODOS WHERE PK_OTACTICO = '$id' AND PK_PERIODO = '$periodo'";
		$num = database::getNumRows($sql);  	
		return $num;
		}
		...
		$sql = "SELECT PK1, PK_USUARIO, FECHA_R, COMENTARIO FROM PL_POPERATIVOS_OBJETIVOST_COMENTARIOS_PERIODOS WHERE PK_OTACTICO = '$id' AND PK_PERIODO = '$periodo' ORDER BY FECHA_R DESC";	
		$rows = database::getRows($sql);
	   ...
	   foreach($rows as $row){
		...
		$sql = "SELECT PK1, PK_USUARIO, FECHA_R, COMENTARIO FROM PL_POPERATIVOS_OBJETIVOST_COMENTARIOS WHERE PK_OTACTICO = '$id' ORDER BY FECHA_R DESC";	
	   ...
	   foreach($rows as $row){
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_RESUMENE_COMENTARIOS_SEGUIMIENTO WHERE PK_POPERATIVO = '$idoperativo' AND PK_PERIODO = '$idperiodo' AND TIPO = '$tipo' ";	
		...
		$this->comentarios = array(); 
		$sql = "SELECT PK1, PK_USUARIO, FECHA_R, COMENTARIO FROM PL_POPERATIVOS_RESUMENE_COMENTARIOS_SEGUIMIENTO WHERE PK_POPERATIVO = '$idoperativo' AND PK_PERIODO = '$idperiodo' AND TIPO = '$tipo' ORDER BY FECHA_R DESC";	
	   ...
	   foreach($rows as $row){
		...
		$sql = "SELECT PK1, NOMBRE, PUESTO, DEPARTAMENTO, VALORACION FROM PL_POPERATIVOS_COLABORADORES WHERE PK_OPERATIVO = '$idpan' AND PK_PERIODO = '$idperido' ORDER BY FECHA_R DESC";	
	...
	function eliminarComentarioResumenPeriodos($id_comentario){
     ...
     function esperiodoActivo($id,$plan){
		...
		$sql = "SELECT ENVIADO, ORDEN FROM PL_POPERATIVOS_PERIODOS WHERE PK1='$id' AND PK_POPERATIVO = '$plan' ";
     ...
     function getComentariosResumen($id){
        ...
        $sql = "SELECT PK1, PK_USUARIO, FECHA_R, COMENTARIO FROM PL_POPERATIVOS_RESUMENE_COMENTARIOS WHERE PK_POPERATIVO = '$id' ORDER BY FECHA_R DESC";	
	    $rows = database::getRows($sql);
...
$sql = "SELECT TOP 1 PK1 FROM PL_POPERATIVOS_RESUMENE_COMENTARIOS_SEGUIMIENTO WHERE PK_USUARIO = '$usuario' AND PK_POPERATIVO = '$idplan' AND PK_PERIODO = '$idperiodo' AND TIPO = '$tipo' AND FECHA_R = '$fechar' ";  
	    ...
	    function getResumenEjecutivo($id){//NO USA
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_RESUMENE_COMENTARIOS WHERE PK_POPERATIVO = '$id'";	
		$num = database::getNumRows($sql);  	
		return $num; 	
	    }
		...
		function setPorcentajeObjetivo($idObjetivo,$idPeriodo,$valor){
			...
			$sql = "SELECT PK1 FROM PL_POPERATIVOS_OBJETIVOST_AVANCES WHERE PK_OTACTICO = '$idObjetivo' AND PK_PERIODO = '$idPeriodo' ";  
	   ...
	   function setPorcentajeMedio($idMedio,$idPeriodo,$valor){
		 ...
		 $sql = "SELECT PK1 FROM PL_POPERATIVOS_MEDIOS_AVANCES WHERE PK_MEDIO = '$idMedio' AND PK_PERIODO = '$idPeriodo' ";  
   		 $rows = database::getNumRows($sql); 
		...
		$sql = "SELECT PORCENTAJE FROM PL_POPERATIVOS_OBJETIVOST_AVANCES WHERE PK_OTACTICO = '$idObjetivo' AND PK_PERIODO = '$idPeriodo'";   
		$row = database::getRow($sql);
		...
		function getPorcentajeObjetivoAnterior($idObjetivo,$idplan,$orden){//NO SE USA?
	  
	  
	    $orden = (int)$orden -1;
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$idplan' AND ORDEN='$orden'";   
		$row = database::getRow($sql);
		...
		$sql = "SELECT PORCENTAJE FROM PL_POPERATIVOS_MEDIOS_AVANCES WHERE PK_MEDIO = '$idMedio' AND PK_PERIODO = '$idPeriodo'";
		$row = database::getRow($sql);
	   ...
	   foreach($rows as $row){	
		...
		$sql = "SELECT FECHA_I FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$idplan' AND ENVIADO = '2'";
		$periodoactivo = database::getRow($sql);
		
	   //   $periodoactivof = date('Y-m-d', strtotime($periodoactivo->format('Y-m-d')));
	   
	   if($periodoactivo['FECHA_I'] == null ||$periodoactivo['FECHA_I'] == ""){ $fechap = $periodoactivo['FECHA_I']; }
	   else{ $fechap = $periodoactivo['FECHA_I']->format('Y-m-d');}
		...
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$idplan'";
		$total = database::getNumRows($sql);      
		
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$idplan' AND ENVIADO = '3'";
		$terminados = database::getNumRows($sql);      
		
		$sql = "SELECT PK1 FROM PL_POPERATIVOS_PERIODOS WHERE PK_POPERATIVO = '$idplan' AND ENVIADO = '1'";
		...
		if($total==$terminados){
		  ...
		  function getResumenArchivoPeriodo($plano,$idPeriodo){
		    ...
		    $sql = "SELECT ADJUNTO FROM PL_POPERATIVOS_RESUMENE_PERIODOS WHERE PK_POPERATIVO = '$plano' AND PK_PERIODO = '$idPeriodo' ";  
		  ...
		  function getResumenPeriodo($plano,$idPeriodo){
		    ...
		    $sql = "SELECT RESUMEN FROM PL_POPERATIVOS_RESUMENE_PERIODOS WHERE PK_POPERATIVO = '$plano' AND PK_PERIODO = '$idPeriodo' ";  
	    ...
	    $result = database::executeQuery($sql);
		 ...
		 function EditFile(){
			...
			$this->campos = array('EVIDENCIA'=>str_replace("'","",$this->titulo),
		...
		$this->campos = array('PK1'=>uniqid($this->tipo),
	                         'EVIDENCIA'=>str_replace("'","",$this->titulo),
							 'ORDEN'=>$orden,
							 'DESCRIPCION'=>$this->descripcion,
							 'AUTOR'=>$this->autor,
							 'TIPO'=>$this->tipo,
...
$mail->Subject = 'Informe enviado para revisi�n';
$mail->Body    = '
...
.'portal/img/logo.png"  alt="Red de Universidades An�huac" border="0"></td>
				</tr>
			</table>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">El plan estar� listo para la elaboraci�n del informe del siguiente periodo seg�n el calendario de la RUA.</p>
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;">Por favor vaya a la p�gina de inicio de sesi�n del Sistema de Planeaci�n e ingrese con su usuario y contrase�a:  </p>
