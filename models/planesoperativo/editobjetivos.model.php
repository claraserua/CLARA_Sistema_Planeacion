...
class editobjetivosModel {
	...
	function getIndicadoresByObjetivo($id){
		...
		//echo '<input type="hidden" value="'.$sql.'"></input>';
		foreach($rows as $row){
			$this->indicadores[] = $row;
        }
	}
	function getIndicadoresByObj($idResul,$idObj){
		$this->indicadoresO = array();
    
		$sql = "SELECT PK_INDICADORMETA FROM PL_POPERATIVOS_OBJETIVOS_INDICADORES WHERE PK_OTACTICO = '$idResul' AND PK_OESTRATEGICO = '$idObj'";	
	    $rows = database::getRows($sql);
	...
	function getObjetivosTacticos($idPLAN,$idlINEA)
	{
		...
		$sql = "SELECT A.ROL,U.PK1,U.APELLIDOS, U.NOMBRE,A.PK_POPERATIVO 
		FROM PL_POPERATIVOS_ASIGNACIONES A, PL_USUARIOS U ,PL_ROLES_PERMISOS  R
		WHERE U.PK1 = A.PK_USUARIO 
		and A.ROL = R.PK_ROL
		AND A.PK_POPERATIVO = '$planoperativo'
		AND R.PK_PERMISO = 'P129'";
	    $rows = database::getRows($sql);
...
$mail->Subject = 'Plan operativo enviado a revisi�n';
$mail->Body    = '
<html xmlns="http://www.w3.org/1999/xhtml">
<head>
<META NAME="ROBOTS" CONTENT="NOINDEX, NOFOLLOW">
<meta http-equiv="Content-Type" content="text/html; charset=ISO-8859-1" />
<title>Sistema de Planeaci�n RUA</title>
...
<p style="font-family:Segoe UI, Tahoma, sans-serif; font-size:10pt; color:#000;"> El plan operativo: <strong>'.$operativo.'</strong> ha sido enviado para su revisi�n por el usuario:  </p>
...
<div style="margin-top:20px;font-family:\'Segoe UI\', Tahoma, sans-serif; font-size:10pt; line-height:13pt; color:#000;"> 
                  ...
                  <ul style="font-family:Segoe UI, Tahoma, sans-serif;color:#000;font-size:10pt;">
                    <li>Puede comentar en cada resultado del plan, en el diagn�stico inicial y agregar comentarios generales.</li>

                    <li>S�lo los usuarios que tengan permiso podr�n enviar el plan revisado al centro.</li>
                  </ul>
...
Secretar�a Ejecutiva de la Red de Universidades An�huac</p>
					</td>
				</tr>
			</table>
...
//}
	 ...
	 function getUsuarioResultado($id){
