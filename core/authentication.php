...
class Authentication extends Session{
	...
	function  validarUsuario($user,$pass){
		...
		function getPrivilegio2($plan){
		
			$user = $_SESSION['session']['user'];
			$permisos = array();
			$permisos2 = array();
			$sql = "SELECT DISTINCT P.PK_PERMISO
					FROM PL_POPERATIVOS_ASIGNACIONES as A, PL_ROLES_PERMISOS as P
					WHERE A.ROL = P.PK_ROL
					AND A.PK_POPERATIVO = '$plan' 
					AND A.PK_USUARIO = '$user'";
			$rows = database::getRows($sql);
			foreach($rows as $row){
				//$permisos2[] = $row;
				array_push($permisos,$row['PK_PERMISO']);
			}
			/*foreach($permisos2 as $row){
				array_push($permisos,$row['PK_PERMISO']);
			}*/
			return $permisos;
		}
