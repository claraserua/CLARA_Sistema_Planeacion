...
var s= parseInt(gup('s'));
  ...
  if(filter.length>0){
   ...
   idNivel = gup('Nivel');
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=jerarquia&method=Buscar&Nivel="+idNivel+""+urltag,  
...
function EliminarNivel(id,confirm){
      
	 if(!confirm){
	 $('#titlemodal').html('Eliminar Nivel');
	 $('#bodymodal').html('¿Esta seguro de eliminar el nivel? <br><br>Se eliminaran todos los subniveles de este Nodo');
	  $("#aceptarmodal").attr("onClick", "javascript:EliminarNivel('"+id+"',true);return false;")
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=jerarquia&method=Eliminar&id="+id,  
    success: function(msg){  
   ...
   buscar();
     ...
     // $('div.table').unblock();
	  ...
	  $('html, body').animate({scrollTop:0}, 10);
				...
				$("#bodyAlerta").html("Se ha eliminado el nivel"); 
