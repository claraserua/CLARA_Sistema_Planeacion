...
var s= parseInt(gup('s'));
  ...
  urltag = "&p="+gup('p')+"&s="+gup('s')+"&sort="+gup('sort')+"&q="+q;
   ...
   window.location.hash = urltag;
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=historicope&method=Buscar"+urltag,  
...
function DesArchivarPlanEstrategico(id,confirm){
      
	 if(!confirm){
	 $('#titlemodal').html('Desarchivar Plan Estrategico');
	 $('#bodymodal').html('¿Esta seguro de Desarchivar el plan');
	  $("#aceptarmodal").attr("onClick", "javascript:DesArchivarPlanEstrategico('"+id+"',true);return false;")
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=historicope&method=DesArchivar&id="+id,  
    success: function(msg){  
    ...
    buscar();
	  ...
	  $('html, body').animate({scrollTop:0}, 10);
                ...
                $("#alerta").addClass("alert alert-success");
				$("#headAlerta").html("¡Correcto!");
				$("#bodyAlerta").html("Se ha desarchivado el plan estrategico"); 
