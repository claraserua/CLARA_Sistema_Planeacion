...
//var array_debilidades = new Array();//new

//var array_oportunidades = new Array();//new
...
*/
	...
	var resultado = '<li id="RESULTADO"> <!--====================OBJETIVO=====================--> ';
		...
		resultado += '<div class="input-prepend"><p>Seleccione un Objetivo Estratégico</p></div>';
	...
	function getAndDelIndicadoresByObj(idResultado, obj){
		...
		//console.log("numeroObjetivo",numeroObjetivo);
		var indi  = "";
		if(valObj != "0"){
			$.ajax({ 	 	 
				type: "POST",  
				...
				success: function(msg){
					...
					/*if(resultado.length > 0){
						resultado.forEach(function(objI) { 
							if(objI.INDICADOR.toString() === "" || objI.INDICADOR === null){
								indi +='<p>No existen Indicadores para este Objetivo Estratégico.</p>';
	
							}else{
								//console.log("jiij",objI.INDICADOR.toString());
								indi += '<div class="input-prepend INDdiv" style="width:50%;background: #ffffff;;border-bottom: 1px solid #BBBBBB;padding:4px;border-radius:4px;">'
								+'<span id="textindicador111" class="" style="display: inline-block;margin-bottom: -1px;">'
									+'<input id="IND-'+idResultado+'-'+objI.PK1+'" idObje="'+idResultado+'"  idObjeEstra="'+valObj+'"type="checkbox" value="'+objI.PK1+'" name="" style="margin-bottom:5px;margin-right:15px;">'
								+'</span>'
								+'<span class="add-on">'+numeroObjetivo+'.'+(objI.ORDEN+1)+'</span>'
								+'<p style="margin-left:3px;height:auto;display:inline-block;" name="L1-objetivo" readonly="">'+objI.INDICADOR+'</p>'
								+'</div>';
							}
 ...
 $.ajax({ 	 	 
	...
	var getObjetivosEstrategicos= $.ajax({ 	 	 
		type: "POST",  
	...
	$.when( getResponsables, getObjetivosEstrategicos  ).done(function ( v1, v2 ) {}); 
...
function InsertMedio(idResultado){
   ...
   function InsertIndicadorMeta(idResultado){
	...
	var evidencia = '<li id="EVIDENCIA"> <!--====================EVIDENCIA=====================--> ';
         ...
         evidencia += '<button class="btn btn-small" id="SAVE-EVIDENCIA" ><i class="icon icon-save"></i>Salvar Evidencia</button>';				
		 evidencia += '</div>';         
         evidencia += '</td>';
         evidencia += '</tr>';
...
function UpdateArea(id){//Debilidades
...
function EliminarArea(id){//Debilidades
...
function AgregarArea(){//Debilidades
...
function InsertArea(){//Debilidades
