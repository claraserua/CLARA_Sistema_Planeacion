 ...
 $('div.table').block({ css: { backgroundColor: 'transparent', color: '#EC5C00', border:"null" },
  ...
  if(gup('filter')){
   ...
   window.location.hash = urltag;
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=historicopo&method=Buscar"+urltag,  
 ...
 $("#page_size_200-panel").addClass("page_size_200-selected");
  ...
  /*if(filter.length>0){
   ...
   window.location.hash = urltag;
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=historicopo&method=BuscarPlanesEstrategicos"+urltag,  
    success: function(msg){  
...
function ExportarExcel(id,idestretegico){
		...
		var caracteristicas = "height=300,width=830,scrollTo,resizable=1,scrollbars=1,location=0";
      	...
      	myWindow.document.body.innerHTML = '<div align="center" style="margin-top:15px; font-family:Arial, Helvetica, sans-serif;"><img src="http://redanahuac.mx/app/serviciosocial/skins/default/images/spinner-md.gif"/><br /><br /><span>Generando Excel espere por favor...</span></div>';
...
function EliminarPlanOperativo(id,confirm){
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=historicopo&method=Eliminar&id="+id,  
    success: function(msg){  
...
function DesarchivarPlanOperativo(id,confirm){
      
	  $("#alerta").hide();
	  
	 if(!confirm){
	 $('#titlemodal').html('Desarchivar Plan Operativo');
	 $('#bodymodal').html('¿Esta seguro de desarchivar el plan? Se colocara en los Planes Operativos Activos');
	  $("#aceptarmodal").attr("onClick", "javascript:DesarchivarPlanOperativo('"+id+"',true);return false;")
    ...
    type: "GET",  
    url: "index.php",  
    data: "execute=historicopo&method=Desarchivar&id="+id,  
    success: function(msg){  
     ...
     // $('div.table').unblock();
	  ...
	  $('html, body').animate({scrollTop:0}, 10);
                ...
                $("#alerta").addClass("alert alert-success");
				$("#headAlerta").html("¡Correcto!");
				$("#bodyAlerta").html("Se ha DESARCHIVADO el plan operativo"); 
