	...
	({
		...
		post : function ()
		{
			...
			if ($('input[type=file]').val().length)
			{
                ...
                $.blockUI({ css: { backgroundColor: 'transparent', color: '#fff', border:"null" },
                 message: '<img src="skins/admin/images/ajax-loader.gif" /><br><h3> Guardando Tutorial</h3>'
                 }); 
		...
		//alert(response);
			...
			else
			{
				...
				$.unblockUI();
				$('#alerta')
					.html('El tutorial ha sido guardado correctamente.')
					.removeClass()
...
var id = id;
var eliminar = confirm("¿Esta seguro de eliminar este tutorial?");
if(eliminar){
...
$.ajax({ 
    type: "GET",  
    url: "index.php",  
    data: "execute=tutoriales&handle=tutoriales&action=get&method=deleteTutorial&id="+id,  
    success: function(msg){  
