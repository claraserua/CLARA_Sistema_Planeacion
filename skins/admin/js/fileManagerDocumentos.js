...
$('#frmdocumentos').iframePostForm
	({
		...
		post : function ()
		{
			...
			if ($('#txttitulodoc').val().length)
			{
				//GUARDANDO ARCHIVOS
				$('html, body').animate({scrollTop:0}, 'slow');
                $.blockUI({ css: { backgroundColor: 'transparent', color: '#fff', border:"null" },
                 message: '<img src="skins/admin/images/ajax-loader.gif" /><br><h3> Guardando Documento</h3>'
                 }); 
			...
			else
			{
				...
				$('#alerta')
					.html('Por favor ingresa el titulo del Documento')
					.css({
						color : '#9c0006',
						background : '#ffc7ce',
		...
		//alert(response);
			...
			else
			{
				...
				$.unblockUI();
				$('#alerta')
					.html('El documento ha sido guardado correctamente.')
					.removeClass()
